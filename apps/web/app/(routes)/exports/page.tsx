...
import { getPersistedExportJobs } from "@/lib/persistence";

export default async function ExportsPage() {
  const result = await getPersistedExportJobs();
  const jobs = result.data as Array<{ id: string; kind: string; status: string; created_at?: string; createdAt?: string }>;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {["products", "trending", "competitors", "watchlist", "latest_scan"].map((kind) => (
          <Card key={kind}>
      ...
      <Card>
        ...
        {result.warning ? <div className="mb-4 text-sm text-amber-300">{result.warning}</div> : null}
          ...
          {jobs.map((job) => (
            <div key={job.id} className="flex items-center justify-between rounded-2xl border border-white/5 bg-white/[0.04] px-4 py-3">
              <div>
                ...
                <div className="text-xs text-slate-400">{job.created_at ?? job.createdAt}</div>
              </div>
              <div className="rounded-full border border-emerald/30 bg-emerald/10 px-3 py-1 text-xs text-emerald">{job.status}</div>
            </div>
          ))}
          {jobs.length === 0 ? <div className="text-sm text-slate-500">Belum ada export job tercatat.</div> : null}
        </div>
      </Card>
    </div>
  );
